...
public class Presenter{
    ...
    Presenter (){
        ...
        model = new Model(); 
        //playerEntry = new PlayerEntryScreen(this); // I think we must pass the presenter into the player entry screen constructor so 
    ...
    public void startApp(){
        ...
        splash.hideSplash();  
        //playerEntry.show(); // not sure if this is how we plan on showing the next screen make sure all functionality is off if not shown
    }
    ...
    void searchDataBaseForPlayer (String a){
        ...
        try {
            if(model.Search(a)){
                codeName = model.getCodeName(); // I would prefer for the model to be able to flat out get the codename using the id, but this should still be fine
                //either store this codename as a variable or pass it straight into player entry 
            }
            else {
